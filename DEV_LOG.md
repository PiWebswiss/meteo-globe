...
I added an exam preflight helper script at scripts/exam-check.sh. It validates the running stack with one command (bash scripts/exam-check.sh) by checking app homepage, /api/config, and tile endpoint health (accepting HTTP 200 or HTTP 404 at z0 for regional imports), so Raspberry Pi demo readiness can be confirmed quickly before presentation.I removed the temporary exam preflight script and its README references to keep operations strictly to docker compose commands only.